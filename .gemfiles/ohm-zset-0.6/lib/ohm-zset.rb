  ...
  class ZSet < Struct.new(:key, :namespace, :model, :score_field)
    ...
    class << self
      def intersect_multiple(new_key, sets, weights = [])
        ...
        Ohm.redis.call('zinterstore',new_key, sets, :weights => weights)
        new_set
      end
      ...
      def union_multiple(new_key, sets, weights = [])
        ...
        Ohm.redis.call('zunionstore',new_key, sets, :weights => weights)
    ...
    def count(a = '-inf', b = '+inf')
      ...
      redis.call('zcount',key, a, b)
    end

    def ids(a = 0, b = -1)
      execute { |key| redis.call('zrange',key, a, b) }
    end
    ...
    def range_with_score(a = 0, b = -1)
      _return = process_range_with_score { redis.call('zrange',key, a, b, with_scores: true) }
    end

    def revrange(a = 0, b = -1)
      fetch(execute { |key| redis.call('zrevrange',key, a, b) })
    end

    def revrange_with_score(a = 0, b = -1)
      _return = process_range_with_score { redis.call('zrevrange',key, a, b, with_scores: true) }
    end
    ...
    def add(model, custom_score = nil)
      ...
      if custom_score.nil?
        redis.call('zadd',key, lambda_function.call(score_value), model.id)
      else
        redis.call('zadd',key, lambda_function.call(custom_score), model.id)
      end
    end
    ...
    def delete(model)
      redis.call('zrem',key, model.id)
      model
    end

    def remrangebyrank(a, b)
      redis.call('zremrangebyrank',key, a, b)
    end

    def remrangebyscore(a, b)
      redis.call('zremrangebyscore',key, a, b)
    end
    ...
    def intersect(set, w1=1.0, w2=1.0)
      ...
      redis.call('zinterstore',new_set.key, [key, set.key], :weights => [w1, w2])
      new_set
    end
    ...
    def intersect_multiple(sets, weights = [])
      ...
      redis.call('zinterstore',new_set.key, sets, :weights => weights)
    ...
    def intersect_multiple!(sets, weights = [])
      weights = [1.0] * sets.length if weights == []
      redis.call('zinterstore',key, sets.map(&:key), :weights => weights)
      self
    end
    ...
    def union(set, w1=1.0, w2=1.0)
      ...
      redis.call('zunionstore',new_set.key, [key, set.key], :weights => [w1, w2])
    ...
    def union_multiple(sets, weights = [])
      ...
      redis.call('zunionstore',new_set.key, sets, :weights => weights)
    ...
    def revrank(model)
      redis.call('zrevrank',key, model.id)
    end

    def score(model)
      redis.call('zscore',key, model.id).to_i
    end
    ...
    def rangebyscore(a = '-inf', b = '+inf', limit = {})
      ...
      fetch(execute { |key| redis.call('zrangebyscore',key, a, b, :limit => [limit[:offset], limit[:count]]) })
    end
    ...
    def rangebyscore_with_score(a = '-inf', b = '+inf', limit = {})
      ...
      _return = process_range_with_score { redis.call('zrangebyscore',key, a, b, with_scores: true, :limit => [limit[:offset], limit[:count]]) }
    end
    ...
    def revrangebyscore(a = '+inf', b = '-inf', limit = {})
      ...
      fetch(execute { |key| redis.call('zrevrangebyscore',key, a, b, :limit => [limit[:offset], limit[:count]]) })
    end
    ...
    def revrangebyscore_with_score(a = '+inf', b = '-inf', limit = {})
      ...
      _return = process_range_with_score { redis.call('zrevrangebyscore',key, a, b, with_scores: true, :limit => [limit[:offset], limit[:count]]) }
    end
    ...
    def save_set
      redis.hmset('ZSet:' + key, *get_hmset_attrs)
    end
